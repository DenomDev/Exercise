<script>
    ...
    export let auth, userData, usersData;
</script>

<nav class="bg-gray-800 m-0 mb-4">
                            ...
                            <button
                                ...
                                {:else}
                                    <svg
                                        ...
                                        xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 448 512"
                                        class="h-6 w-6 svg-inline--fa fa-user fa-w-14 fa-spin fa-lg"><path
                                            fill="currentColor"
        ...
        <table class="min-w-full bg-white">
            <tr class="bg-gray-800 text-white">
                {#each Object.entries(userData).sort(compare) as value}
                    <th class="uppercase font-semibold text-sm">{value[0]}</th>
                {/each}
            </tr>
            <tr class="text-gray-700">
                {#each Object.entries(userData).sort(compare) as value}
                    <td class="text-left py-3 px-4">{value[1]}</td>
                {/each}
            </tr>
        </table>
    </section>
        ...
        <table class="min-w-full bg-white">
            <tr class="bg-gray-800 text-white">
                {#each Object.entries(usersData[0]).sort(compare) as value}
                    {#if value[0] != 'ref'}
                        <th
                            class="sticky top-0 bg-gray-800 text-white z-30 uppercase font-semibold text-sm">
                            {value[0]}
                        </th>
                    {/if}
                {/each}
